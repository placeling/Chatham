...
class OauthController < ApplicationController
  ...
  def access_token_with_xauth_test
    ...
    if params[:x_auth_mode] == "client_auth"
      ...
      access_token = current_token && current_token.exchange!
      user = access_token.user
      if send_json
        render :json => {:status => "success", :token => access_token, :user => user.as_json({:current_user => user, :perspectives => :created_by})}
      else
        render :text => access_token.to_query
      end
    end
