...
class PlacesController < ApplicationController
  ...
  def search
    ...
    if lat && lng && params[:query] && params[:query].length > 0
      ...
      if query.length > 0
        ...
        raw_places = gpa.suggest(lat, lng, query)

        raw_places.each do |raw_place|
          # Need to remove "political" and "route" types to stay in sync with mobile client
          if !raw_place.types.include?("route")

            place = Place.find_by_google_id(raw_place.id)

            if place.nil?
              #not here, and we need to fetch it
              gp = GooglePlaces.new
              place = Place.new_from_google_place(gp.get_place(raw_place.reference))
              place.user = current_user unless current_user.nil?
              place.client_application = current_client_application unless current_client_application.nil?
              place.save!
            end
  ...
  def show
    ...
    end if @all_perspectives = @place.perspectives.entries
